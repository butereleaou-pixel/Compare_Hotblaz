...
def compare_chat(user_input: str, db_path: str, correct_answer = "A", index = None) -> str:
    ...
    time.sleep(0.5)
    loading_animation("open thinking mode", 12)# wait for database to wipe out, log writing or temporary interrupt.
    #_________________________________________________________________________________________
    try:
    ...
    **{picked_ans}**
    ...
    - Mentioning any other options
    - Referring to A/B/C/D in ANY form
    - Comparing with other choices
    - Talking about alternative answers
    DO NOT EVEN IMPLY they exist.
    3. Only use code blocks if strictly necessary.
    4. At the END, state ONLY the full final answer as a complete sentence.
    DO NOT use any letter label (A/B/C/D).
    5. No uncertainty, no extra commentary, only clean Markdown.

    Now produce reasoning that ONLY supports {picked_ans} as if it is the only possible conclusion.
    """

    result_answer = call_api(prefix_answer, user_input)
    print(picked_ans)
    ...
    return picked_ans, result_answer
