...
def analyze_answer_list(answer_list_str, average_eucli_dis):
    ...
    for ans, data in stat.items():
        ...
        result[ans] = {
            "dis": round(dis, 6),
            "count": data["count"]  # 保留 count 用于排序
        }
    ...
    ###########################################################################
    # ✅【唯一修改】这里从按 dis 排序 → 改为按 count 降序取 top2
    ###########################################################################
...
# ===================== 处理单个 DF =====================
def process_single_df(raw_df, config_path='../config_adjust.json'):
    try:
        # 稳定获取文件名
...
# ===================== 批量处理 =====================
def run_batch_analysis(raw_dfs):
    SPECIAL_VALUE = 2026411151850

    for df in raw_dfs:
        if (df["eucli_dis"] == SPECIAL_VALUE).any():
            return SPECIAL_VALUE

    return [process_single_df(df) for df in raw_dfs]